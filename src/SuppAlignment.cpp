...
namespace sophia {
    ...
    /** Parse the supplementary alignment information from an SA:Z: tag according to the a
      ...
      */
    SuppAlignment SuppAlignment::parseSamSaTag(std::string::const_iterator saCbegin,
                                               std::string::const_iterator saCend,
                                               bool primaryIn,
                                               bool lowMapqSourceIn,
                                               bool nullMapqSourceIn,
    ...
    //	std::cerr << std::endl;
        ...
        try {
            ...
            e <<
                error_info_string("from = " + std::string(fieldBegins[RNAME], fieldEnds[RNAME]));
            throw e;
        }
        ...
        if (cigarChunks.size() != 0) {
            ...
            if (bpChrIndex == result.chrIndex) {
                result.matchFuzziness = std::min((ChrSize) abs(static_cast<long>(bpPos) - static_cast<long>(result.pos)), result.matchFuzziness);
            }
        }
    ...
    /** The syntax parsed here is the same as the one generated by `SuppAlignment::print()`.
      * TODO FIX: This parser parses the same syntax as `SuppAlignmentAnno::parseSaSupport(const std::string&)`.
      */
    SuppAlignment SuppAlignment::parseSaSupport(const std::string& saIn) {
        try {
            const ChrConverter &chrConverter = GlobalAppConfig::getInstance().getChrConverter();
            ...
            try {
                ...
                e <<
                    error_info_string("from = " + std::string(next(saIn.cbegin(), index), saIn.cend()));
                throw e;
            }
