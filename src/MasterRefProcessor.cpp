...
namespace sophia {
    ...
    using namespace std;
MasterRefProcessor::MasterRefProcessor(const vector<string>& filesIn, const string &outputRootName, const string &version, const int defaultReadLengthIn) :
				NUMPIDS { static_cast<int>(filesIn.size()) },
				DEFAULTREADLENGTH { defaultReadLengthIn },
				mrefDb { } {
	const vector<int> CHRSIZES { 249250622, 243199374, 198022431, 191154277, 180915261, 171115068, 159138664, 146364023, 141213432, 135534748, 135006517, 133851896, 115169879, 107349541, 102531393, 90354754, 81195211, 78077249, 59128984, 63025521, 48129896, 51304567, 155270561, 59373567, 106434, 547497, 189790, 191470, 182897, 38915, 37176, 90086, 169875, 187036, 36149, 40104, 37499, 81311,
...
//		mrefDbPtrs.emplace_back(CHRSIZES[i] + 1, nullptr);
	...
	mergedBpsOutput = make_unique<ofstream>(outputRootName + "_" + strtk::type_to_string<int>(NUMPIDS) + "_mergedBpCounts.bed");
	short fileIndex { 0 };
	for (const auto& gzFile : filesIn) {
		...
		++fileIndex;
		cerr << gzFile << "\t" << diff.count() << "\t" << newBreakpoints << "\t" << fileIndex << "\t" << 100 * (fileIndex + 0.0) / NUMPIDS << "%\n";
	}
	auto defuzzier = DeFuzzier { DEFAULTREADLENGTH * 3, true };
	auto i = 84;
	while (!mrefDb.empty()) {
		mrefDb.back().erase(remove_if(mrefDb.back().begin(), mrefDb.back().end(), [](const MrefEntry& bp) {return bp.getPos()==-1;}), mrefDb.back().end());
...
unsigned long long MasterRefProcessor::processFile(const string& gzPath, short fileIndex) {
