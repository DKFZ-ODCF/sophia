...
namespace sophia {
    ...
    namespace hg38 {
        ...
        static const vector<int> chrSizesCompressedMref {
        ...
        static const vector<int> indexConverter {
    ...
    const std::string Hg38ChrConverter::assembly_name = "hg38";
    ...
    int Hg38ChrConverter::readChromosomeIndex(std::string::const_iterator startIt,
                                              char stopChar) const {
        int chrIndex {0};
        if (isdigit(*startIt)) {
            for (auto chr_cit = startIt; *chr_cit != stopChar; ++chr_cit) {
                // Parse the ASCII sequence into a number.
