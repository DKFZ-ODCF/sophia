...
namespace sophia {
    ...
    const std::string SuppAlignmentAnno::STOP_CHARS = "|(\t";
    ...
    /** The syntax parsed here is the same as the one generated by `SuppAlignment::print()`.
      *  TODO FIX: This parser parses the same syntax as `SuppAlignment::parseSaSupport(const std::string&)`.
      */
        ...
        try {
            ...
            for (; saStrIn.at(index) != '('; ++index) {
                if (saStrIn.at(index) == '-') {
                    ...
                    if (!fuzzy) {
                        pos = 10 * pos + static_cast<int>(saStrIn.at(index) - '0');
                    } else {
                        extendedPos = 10 * extendedPos + static_cast<int>(saStrIn.at(index) - '0');
                    }
                }
            }
            ...
            // The following is different in SuppAlignment::parseSaSupport(const std::string &)
            // start of difference
            ...
            // This is almost the same as in SuppAlignment::parseSaSupport(const std::string &),
            // but in here, strictFuzzyCandidate is a member variable, while in
            // SuppAlignment::parseSaSupport(const std::string &), it is just a local variable.
        ...
        : chrIndex{emittingBpChrIndex},
          pos{saAnnoIn.isDistant()
                  ? static_cast<int>(
                        std::max(1,
                                 static_cast<int>(
                                     round(emittingBpPos - static_cast<ChrSize>(1.5 * DEFAULT_READ_LENGTH)))
                                 )
                    )
                  : emittingBpPos},
          extendedPos{
              saAnnoIn.isDistant()
                  ? emittingBpPos + static_cast<ChrSize>(1.5 * DEFAULT_READ_LENGTH)
                  : emittingBpPos},
          support{0},
          secondarySupport{0},
          mateSupport{0},
        ...
        if (chrIndex == rhs.getChrIndex()) {
            if (strictFuzzy || rhs.isStrictFuzzy()) {
                ...
                if (rhs.getPos() >= extendedPos) {
                    return (static_cast<long>(rhs.getPos()) - static_cast<long>(extendedPos)) <= fuzziness;
                }
                return (static_cast<long>(pos) - static_cast<long>(rhs.getExtendedPos())) <= fuzziness;
